...
InfluxdbBackend.prototype.httpPOST_v09 = function (points) {
  ...
  for( var i=0; i<points.length; i++){
    var measurement = points[i].measurement;
    var value = points[i].fields.value;
    var arr = measurement.split('.');
    if (arr[0] != 'airflow') {
      continue;
    }
    var line = 'airlfow,type='+arr[1];
    var raw_line = measurement+" "+value;
    if (arr[1] == 'dag') {
      if (arr[4] == 'duration') {
        // airflow.dag.ofood_source_sqoop.check_table_ods_sqoop_base_jh_waimai_df.duration.timer.median 1.535472
        line += ",dag_id="+arr[2]+",task_id="+arr[3]+",key="+arr[4]+",index="+arr[5]+" "+arr[6]+"="+value+"\n";
      }
      else if (arr[2] == 'loading-duration'){
        // airflow.dag.loading-duration.app_import_bi_mysql_monthly.timer.median 1.899806
        line += ",dag_id="+arr[3]+",key="+arr[2]+",index="+arr[4]+" "+arr[5]+"="+value+"\n";
      } else {
        self.log(raw_line);
      }
    } else if (arr[1] == 'dagrun') {
      // airflow.dagrun.dependency-check.dwd_oride_order_trip_raw_feature_hi.timer.median
      // airflow.dagrun.schedule_delay.bussiness_alert.timer.median 8641.803
      if (arr[2] == 'dependency-check' || arr[2] == 'schedule_delay') {
        line += ",dag_id="+arr[3]+",key="+arr[2]+",index="+arr[4]+" "+arr[5]+"="+value+"\n";
      } else if (arr[2] == 'duration'){
        // airflow.dagrun.duration.success.superset_oride_orders_10min.timer.median 72197.587
        line += ",dag_id="+arr[4]+",key="+arr[2]+",index="+arr[5]+",status="+arr[3]+" "+arr[6]+"="+value+"\n";
      } else {
        self.log(raw_line);
      }
    } else if (arr[1] == 'dag_processing') {
      // airflow.dag_processing.last_run.seconds_ago.ods_sqoop_base_merchant_receive_money_record_di.gauge 0.649783
      // airflow.dag_processing.last_runtime.ofood_source_sqoop.gauge 1.046695
      if (arr[2] == 'last_run' || arr[2] == 'last_runtime') {
        line += ",dag_id="+arr[3]+",key="+arr[2]+" "+arr[4]+"="+value+"\n";
      } else {
        self.log(raw_line);
      }
    } else if (arr[1] == 'executor') {
      // airflow.executor.open_slots.gauge 442
      // airflow.executor.queued_tasks.gauge 10
      // airflow.executor.running_tasks.gauge 70
      line += ",key="+arr[2]+" "+arr[3]+"="+value+"\n";
    } else if (arr[1] == 'pool') {
      // airflow.pool.starving_tasks.default_pool.gauge 0
      // airflow.pool.open_slots.default_pool.gauge 511
      // airflow.pool.used_slots.default_pool.gauge 1
      line += ",key="+arr[2]+",pool_name="+arr[3]+" "+arr[4]+"="+value+"\n";
    } else {
      line += " "+arr[2]+"="+value+"\n";
      // self.log(raw_line);
      //line = line + "type=" + measurement + " value=" + value + "\n";
    }
    p = p + line;
  }
  self.log("write data length" + points.length);
